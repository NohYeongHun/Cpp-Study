#include <iostream>
using std::cout;
using std::endl;

// 오늘의 주제 : 불리언(bool)과 실수

// 불리언(boolean) 참 / 거짓
bool isHighLevel = true;
bool isPlayer = true;
bool isMale = false;


// [Note]
// 사실 bool은 그냥 1바이트 정수에 불과
// 왜 정수 시간에 언급하지 않았는가?
// -> 일리 있음. 어셈블리에서 bool이라는 것은 없음
// bool만 봐도 참/거짓 둘 중 하나라는 힌트를 줌. (가독성)
// 1이면 여자 0이면 남자.

// bool < 1바이트 정수
// - al

// 실수 (부동소수점)
// flaot double
// 3.14
// 쩜 앞/뒤를 기준으로 16bit/16bit씩 끊으면?
// (0 ~ 65535) . (0~65535) 범위가 너무 작다.

// 부동(浮動)소수점 => 소수점이 움직인다.
// .을 유동적으로 움직여서 표현하는 방법

// 3.1415926535 = 0.31415926535 * 10
// 1) 정규화 = 0.31415926535 * 10
// 2) 31415926535 (유효숫자) 1 (지수)

// float 부호(1bit) 지수(8bit) 유효숫자(23) = 32비트 = 4byte
// double 부호(1bit) 지수(11bit) 유효숫자(52) = 64비트 = 8byte
float attackSpeed = -3.375f; // 4byte
double attackSpeed2 = 123.4123; // 8byte

// ex) -3.375라는 값을 저장
// 1) 2진수 변환 = (3) + (0.375) = 0b11 + 0b0.011 = 0b11.011
// 소수점 이진법
// => 소수부에 2를 곱하는데 그 결과가 1로 떨어질 때까지 혹은 똑같은 소수점이 나올때까지 반복
// 0.375 * 2 = 0.75 (0)
// 0.75 * 2 = 1.5 (1)
// 0.5 * 2 = 1.0 (1)
// => 0b011
// 2) 정규화 0b1 | 1011 * 2 ^ 1
// 1(부호) 1(지수) 1011(유효숫자)
// 단 지수는 unsigned byte라고 가정하고 숫자 + 127 만들어줌
// 예상 결과 : 0b 1(부호) 10000000(지수) 1011000 0000 0000 0000 0000

// 프로그래밍할 때 부동소수점은 항상 '근사값'이라는 것을 기억
// 1/3 = 0.3333333333333333333333333333

// 특히 수가 커질 수록 오차 범위도 매우 커짐.
// 실수 2개를 == 으로 비교하는 것은 지양

int main()
{

	// 00007FF7E58318CB  movzx edx,byte ptr [isHighLevel (07FF7E583C000h)] 
	// edx에 byte 크기의 ptr 기입. 
	// 어셈블리에서는 boolean을 1byte로 취급한다.
	cout << isHighLevel << endl; 



}
